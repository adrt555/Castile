...
import { createPurchaseOrder } from "@/app/actions/purchaseOrderActions";
...
export default function CreatePurchaseOrderPage() {
    ...
    const setProduct = (itemId: string, p: any) => {
        updateItem(itemId, {
            productId: p.id, sku: p.sku || '', description: p.name,
            size: p.size || '', sqftPerBox: p.sqftPerBox || 1,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await createPurchaseOrder({
                ...
                notes,
                items: valid.map(i => ({
                    sku: i.sku, description: i.description, boxes: i.boxes,
                    sqftPerBox: i.sqftPerBox, unitCost: i.unitCost, totalLineCost: i.totalLineCost,
                })),
                subtotal, freight: parsedFreight, tax, total,
            });
    ...
    return (
        <form onSubmit={handleSubmit} className="max-w-[1400px] mx-auto space-y-8 pb-32">
            <div className="flex items-center justify-between border-b border-zinc-200 pb-6">
                <div>
                    <Link href="/admin/purchase-orders" className="text-amber-600 hover:text-amber-700 font-medium text-sm mb-2 inline-block">&larr; Back to Purchase Orders</Link>
                ...
                <div className="flex gap-3">
                    ...
                    <button type="submit" disabled={isSaving} className="px-6 py-2.5 bg-amber-500 hover:bg-amber-600 disabled:bg-amber-300 text-white font-bold rounded-xl shadow-sm transition-colors">
                        {isSaving ? "Saving..." : "Generate PO"}
                    </button>
                </div>
            </div>
                    ...
                    <div className="bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input type="text" value={manufacturer} onChange={e => setManufacturer(e.target.value)} className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-amber-500 font-medium" required />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Expected Date</label>
                                <input type="date" value={expectedDate} onChange={e => setExpectedDate(e.target.value)} className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-amber-500 font-medium" />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Notes</label>
                                <textarea value={notes} onChange={e => setNotes(e.target.value)} className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-amber-500 font-medium h-24" placeholder="Internal notes or vendor instructions..." />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="col-span-2 space-y-6">
                    <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm">
                        <div className="p-6 border-b border-zinc-100 flex justify-between items-center bg-zinc-50/50 rounded-t-2xl">
                            ...
                            <button type="button" onClick={() => setItems([...items, makeItem()])} className="text-amber-600 hover:text-amber-700 font-bold text-sm bg-amber-50 px-3 py-1.5 rounded-lg">+ Add Row</button>
                        </div>
                                        ...
                                        <tr key={item.id} className="group relative">
                                            <td className="px-6 py-4 relative z-50">
                                                <div className="relative w-full">
                                                    <input type="text" placeholder="Search product (SKU/Name)..." value={skuMap[item.id]?.query ?? item.sku}
                                                        onChange={e => setSkuMap(prev => ({ ...prev, [item.id]: { query: e.target.value, open: true } }))}
                                                        onFocus={() => setSkuMap(prev => ({ ...prev, [item.id]: { query: prev[item.id]?.query ?? item.sku, open: true } }))}
                                                        ...
                                                        <>
                                                            ...
                                                            <div className="absolute top-full left-0 mt-1 w-[400px] bg-white border border-zinc-200 rounded-xl shadow-xl z-50 overflow-hidden">
                                                                <div className="max-h-64 overflow-y-auto p-2">
                                                                    {products
                                                                        .filter((p: any) => !skuMap[item.id]?.query || p.name.toLowerCase().includes(skuMap[item.id].query.toLowerCase()) || p.sku.toLowerCase().includes(skuMap[item.id].query.toLowerCase()))
                                                                        .slice(0, 8)
                                                                        .map((p: any) => (
                                                                            <button key={p.id} type="button" onClick={() => { setProduct(item.id, p); setSkuMap(prev => ({ ...prev, [item.id]: { query: '', open: false } })); }}
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col items-center gap-2">
                                                    <input type="number" min="0" step="0.01" placeholder="Sqft" value={item.quantitySqft || ''} onChange={e => updateItem(item.id, { quantitySqft: parseFloat(e.target.value) || 0 })}
                                                    ...
                                                    <button type="button" onClick={() => calcRound(item)} disabled={!item.sqftPerBox}
                                                        className="text-[10px] font-bold text-white bg-amber-500 hover:bg-amber-600 disabled:bg-zinc-200 disabled:text-zinc-400 rounded transition-colors px-2 py-1 w-full max-w-[112px]">
                                                        🧮 Round up
                                                    </button>
                                                </div>
                                            </td>
                                            <td className="px-2 py-4">
                                                <div className="flex flex-col gap-1 items-center justify-center">
                                                    <div className={`text-xs font-bold px-2 py-1.5 rounded w-full max-w-[100px] text-center ${item.productId && item.boxes > 0 ? 'bg-blue-50 border border-blue-100 text-blue-700' : 'text-zinc-300'}`}>📦 {item.boxes} Boxes</div>
                                                    <div className={`text-xs font-bold px-2 py-1.5 rounded w-full max-w-[100px] text-center ${item.productId && item.boxes > 0 && item.boxesPerPallet > 0 ? 'bg-amber-50 border border-amber-100 text-amber-700' : 'text-zinc-300'}`}>🏗️ {item.boxesPerPallet ? Math.ceil(item.boxes / item.boxesPerPallet) : 0} Pallets</div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4 text-right font-bold text-zinc-900">${item.totalLineCost.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                                            <td className="px-6 py-4">
                                                {items.length > 1 && (
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className="relative w-32">
                                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400">$</span>
                                        <input type="number" value={freight} onChange={e => setFreight(e.target.value)} className="w-full bg-white border border-zinc-200 rounded-lg pl-8 pr-3 py-1.5 text-right outline-none focus:border-amber-500" />
                                    </div>
                                </div>
